...
확산: 인접한 방향으로 BFS 구현
바람: 특정 방향으로만 탐색하는데, 계속해서 이전 요소를 현재 요소에 덮어 씌운다.
반시계 방향: 현재 열이 맵의 끝이면 위로, 현재 행이 맵의 1이면 왼쪽으로, 현재 행열이 (1, 1) 이면 아래로, 이외는 오른쪽으로. 공기청정기 도착하면 종료.
시계 방향: 현재 열이 맵의 끝이면 아래로, 현재 행이 맵의 끝이면 왼쪽으로, 현재 열이 1, 행이 끝이면 위로, 이외는 오른쪽으로. 공기청정기 도착하면 종료.
*/
